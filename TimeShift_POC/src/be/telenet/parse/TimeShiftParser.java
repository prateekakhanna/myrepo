...
public class TimeShiftParser extends DomParser{
			...
			if (isIdInList(strElementId)) {
				...
				if (versionNode != null && versionNode.getNodeType() == Node.ELEMENT_NODE) 	{
					...
					if (level == 0) {
						...
						getNodeListForTagName_Recur(parentElement.getChildNodes(), xPath, tsExecData);
					}															
				}
	...
	private void getNodeListForTagName_Recur(NodeList nodeList, String xPath, TimeShiftExecutionData execData) {		
		
		if (nodeList != null && xPath.equals(StringUtils.getLastString(xPath))) {
			processTimeShift(nodeList, execData);			
			return;
		}
		
		String xmlTag = StringUtils.getSubstringBySeparator(xPath, 0); 
		for (int i=0; i<nodeList.getLength(); i++) {
			Node n = nodeList.item(i);			
			System.out.println(n.getNodeName());
				if (n.getNodeType() == Node.ELEMENT_NODE && n.getNodeName().equals(xmlTag)) {					
					getNodeListForTagName_Recur(((Element) n).getElementsByTagName(StringUtils.getSubstringBySeparator(xPath, 1)), 
							StringUtils.getRemainingString(xPath),
							execData);
				}
		}
		...
		for (Element e : elementTimeShiftList) {
			...
			if (SHIFTED.equals(getElementAttributeByTag(e, TIMESHIFT_FIELDNAME))) {
				if (getValidityEndDate(e) == null) {
					...
					//Remove PriceSettingId tag so that Id can be generated by EPC.
//					NodeList nlToDel = e.getElementsByTagName("PriceSettingId");
//					if (nlToDel.getLength() == 1) {
//						dupOldEle.removeChild((Element)nlToDel.item(0));
//					}
